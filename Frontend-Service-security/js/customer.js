...
function saveCustomer() {
  ...
  try {
    ...
    $.ajax({
      url: "http://localhost:9000/serviceSecurity/v1/api/customer",
      method: "POST",
      dataType: "json",
...
function loadTypeDocument() {
  $.ajax({
      ...
      error: function (error) {
          console.error("Error in fetching document types:", error);
...
function loadPerson() {
  ...
  $.ajax({
    url: "http://localhost:9000/serviceSecurity/v1/api/person/filter/" + type,
    method: "GET",
    dataType: "json",
    success: function (response) {
      if (response.status && Array.isArray(response.data)) {
...
function deleteById(id) {
  ...
  if (confirm("¿Estás seguro de que quieres eliminar este registro?")) {
    $.ajax({
      url: "http://localhost:9000/serviceSecurity/v1/api/customer/" + id,
      method: "delete",
      headers: {
